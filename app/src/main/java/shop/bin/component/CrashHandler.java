...
import shop.bin.util.ToastUtils;
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        if (isUTMode) {
            ...
            BinApps.getInstance().startActivity(intent);
        ...
        BinApps.getInstance().exitApp();
    }


    private String getDeviceSuperInfo() {
        try {
            PackageInfo pInfo = BinApps.getInstance().getPackageManager().getPackageInfo(BinApps.getInstance().getPackageName(), 0);
